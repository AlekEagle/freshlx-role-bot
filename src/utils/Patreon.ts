...
export async function startCacheRefresh(): Promise<void> {
  ...
  if (cached.length === 0) {
    await getAllMembers();
    // Emit an event that the cache has been refreshed.
...
export async function forceCacheRefresh(): Promise<void> {
  // Stop the current cache refresh.
  stopCacheRefresh();
  // Clear the cache before fetching.
  cached = [];
  // Restart the cache refresh, it will fetch fresh members immediately.
